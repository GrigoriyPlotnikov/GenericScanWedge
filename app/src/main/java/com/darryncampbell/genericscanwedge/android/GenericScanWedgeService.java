...
public class GenericScanWedgeService extends IntentService {
    ...
    */
    protected void onHandleIntent(Intent intent) {
        ...
        } else if (intent.hasExtra(Datawedge.SCANNER_INPUT_PLUGIN)) {
            if (activeProfilePosition == NO_ACTIVE_PROFILE) {
        ...
        } else if (intent.hasExtra(Datawedge.ENUMERATE_SCANNERS)) {
                handleEnumerateScanners();
        } else if (intent.hasExtra(Datawedge.SWITCH_TO_PROFILE)) {
            handleSwitchToProfile(intent.getStringExtra(Datawedge.SWITCH_TO_PROFILE), profiles, activeProfilePosition);
        } else if (intent.hasExtra(Datawedge.SET_CONFIG)) {
            handleSetConfig(intent.getBundleExtra(Datawedge.SET_CONFIG), profiles);
        } else if (intent.hasExtra(Datawedge.GET_PROFILES_LIST)) {
            handleGetProfilesList(profiles);
        }
    }

    private void handleGetProfilesList(ArrayList<Profile> profiles) {
        Intent result = new Intent(Datawedge.RESULT_ACTION);
        String[] list =  profiles.stream().map(new Function<Profile, String>() {
            @Override
        ...
        result.putExtra(Datawedge.RESULT_GET_PROFILES_LIST, list);
